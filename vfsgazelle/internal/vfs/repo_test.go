...
func TestSnapshotRoundTripPersistence(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, filepath.Join(root, "pkg", "foo.txt"), "hello")
	writeTestFile(t, filepath.Join(root, "pkg", "directive.cfg"), "# gazelle:exclude skipped\n")
	...
	file, ok := loaded.File("pkg/foo.txt")
	if !ok {
		t.Fatal("expected foo.txt in loaded snapshot")
	}
	if file.Content != nil {
