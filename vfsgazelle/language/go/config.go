...
Update io_bazel_rules_go to a newer version in your WORKSPACE file.`
	...
	if !gc.moduleMode {
		if repoFileExists(path.Join(rel, "go.mod")) {
			gc.moduleMode = true
		}
	}
	...
	if f != nil {
		...
		if !gc.prefixSet {
			...
			goModRel := path.Join(rel, "go.mod")
			if result, err := activeRepo.GetModel(goModRel, "go/modfile"); err == nil {
				info := result.Model.(goModInfo)
				if info.ModulePath != "" {
					setPrefix(info.ModulePath)
				}
			} else if repoFileExists(goModRel) {
				log.Printf("parsing %s: %s", filepath.Join(c.RepoRoot, filepath.FromSlash(goModRel)), err)
			}
		}
	}
...
func detectNamingConvention(c *config.Config, rootFile *rule.File) namingConvention {
	...
	detectInDir := func(dir, rel string) namingConvention {
		...
		for _, name := range c.ValidBuildFileNames {
			...
			data, err := activeRepo.ReadFile(buildRel)
			if err != nil {
				continue
			}
			fpath := filepath.Join(c.RepoRoot, filepath.FromSlash(buildRel))
	...
	ents, err := repoListDir("")
	if err != nil {
		return importNamingConvention
	}
	for _, ent := range ents {
		if _, err := repoListDir(ent); err != nil {
			continue
		}
		dirName := ent
