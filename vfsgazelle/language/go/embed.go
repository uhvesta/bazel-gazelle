...
func newEmbedResolver(dir, rel string, validBuildFileNames []string, pkgRels map[string]bool, subdirs, regFiles, genFiles []string) *embedResolver {
	...
	for _, subdir := range subdirs {
		err := walkEmbedTreeRepo(rel, subdir, validBuildFileNames, pkgRels, add)
		if err != nil {
...
func walkEmbedTreeRepo(
	rel, subdir string,
	validBuildFileNames []string,
	pkgRels map[string]bool,
	add func(string, bool) *embeddableNode,
) error {
	if activeRepo == nil {
		return fmt.Errorf("no active vfsgazelle repo snapshot")
	}
	var walk func(repoDir, fileRel string) error
	walk = func(repoDir, fileRel string) error {
		base := path.Base(fileRel)
		entries, err := activeRepo.ListDir(repoDir)
		if err == nil {
			...
			for _, name := range validBuildFileNames {
				if _, ok := activeRepo.File(path.Join(repoDir, name)); ok {
					return nil
				}
			}
			add(fileRel, true)
			for _, entry := range entries {
				childRepoDir := path.Join(repoDir, entry)
				childFileRel := path.Join(fileRel, entry)
				if err := walk(childRepoDir, childFileRel); err != nil {
					return err
				}
			}
			return nil
		...
		if _, ok := activeRepo.File(repoDir); ok {
			if !isBadEmbedName(base) {
				add(fileRel, false)
			}
			return nil
	...
	return walk(path.Join(rel, subdir), subdir)
}
