...
import (
	...
	proto "github.com/uhvesta/bazel-gazelle/vfsgazelle/language/proto"
)

func (gl *goLang) GenerateRules(args v3language.GenerateArgs) v3language.GenerateResult {
	var result v3language.GenerateResult
	withRepo(args.Repo, func() {
		var subdirs []string
		for _, subdir := range args.PackageDir.Subdirs() {
		...
		regularFiles := append([]string{}, regularFileNames...)
		...
		for _, sub := range subdirs {
			if sub == "testdata" {
				...
				if !ok {
					...
					if entries, err := args.Repo.ListDir(testdataRel); err == nil && len(entries) > 0 {
						hasTestdata = true
					}
				}
				break
		...
		var res v3language.GenerateResult
		var rules []*rule.Rule
...
func newGenerator(c *config.Config, gc *goConfig, args v3language.GenerateArgs) *generator {
	g := &generator{
		c:                   c,
		gc:                  gc,
		rel:                 args.Rel,
...
func shouldSetVisibility(args v3language.GenerateArgs) bool {
