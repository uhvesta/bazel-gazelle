...
// Options configures one vfsgazelle run.
type Options struct {
...
// Result contains the frozen snapshot and parsed-model cache produced by Run.
type Result struct {
	Snapshot *vfs.Snapshot
	Cache    *vfs.Cache
}
...
// Run executes the full vfsgazelle pipeline against a cold or patched
// snapshot, then returns the frozen snapshot for reuse by later reruns.
func Run(opts Options) (*Result, error) {
	...
	configurers = append(configurers, &vfsgazellewalk.Configurer{})
	...
	phaseStart = time.Now()
	err = vfsgazellewalk.Walk(repoSnapshot, opts.Config, configurers, func(args vfsgazellewalk.FuncArgs) error {
		active := vfsgazellelanguage.Filter(args.Config, opts.Languages)
		...
		for _, lang := range active {
			res := lang.GenerateRules(vfsgazellelanguage.GenerateArgs{
				Config:     args.Config,
				Repo:       args.Repo,
...
// Registry builds a parser registry from the supplied vfsgazelle languages.
func Registry(langs []vfsgazellelanguage.Language) (*vfs.Registry, error) {
	registry := vfs.NewRegistry()
	if err := vfsgazellelanguage.RegisterParsers(registry, langs); err != nil {
		return nil, err
	}
	return registry, nil
}
...
func initConfigurers(c *config.Config, cexts []config.Configurer) error {
	fs := flag.NewFlagSet("vfsgazelle", flag.ContinueOnError)
...
type languageAdapter struct {
	repo *vfs.Snapshot
	lang vfsgazellelanguage.Language
}
...
func (a languageAdapter) Imports(c *config.Config, r *rule.Rule, f *rule.File) []resolve.ImportSpec {
	return a.lang.Imports(vfsgazellelanguage.ImportsArgs{
		Config: c,
		Repo:   a.repo,
		Rule:   r,
		File:   f,
...
func (a languageAdapter) Embeds(r *rule.Rule, from label.Label) []label.Label {
	return a.lang.Embeds(vfsgazellelanguage.EmbedsArgs{
		Repo: a.repo,
		Rule: r,
		From: from,
	})
...
func (a languageAdapter) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, imports interface{}, from label.Label) {
	a.lang.Resolve(vfsgazellelanguage.ResolveArgs{
		Config:  c,
		Repo:    a.repo,
		Index:   ix,
		Rule:    r,
...
func (a languageAdapter) CrossResolve(c *config.Config, ix *resolve.RuleIndex, imp resolve.ImportSpec, lang string) []resolve.FindResult {
	cr, ok := a.lang.(vfsgazellelanguage.CrossResolver)
	if !ok {
		return nil
	}
	return cr.CrossResolve(vfsgazellelanguage.CrossResolveArgs{
		Config: c,
		Repo:   a.repo,
		Index:  ix,
		Import: imp,
