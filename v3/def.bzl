...
DEFAULT_LANGUAGES = [
    Label("//v3/language/proto"),
    Label("//v3/language/go"),
]
...
def _gazelle_v3_binary_impl(ctx):
    go = go_context(ctx)
...
gazelle_v3_binary = rule(
    implementation = _gazelle_v3_binary_impl,
    attrs = {
        "languages": attr.label_list(
            providers = [GoArchive],
            mandatory = True,
            allow_empty = False,
            doc = "List of v3 language libraries exporting NewLanguage().",
        ),
        "_go_context_data": attr.label(default = "@io_bazel_rules_go//:go_context_data"),
        "_stdlib": attr.label(default = "@io_bazel_rules_go//:stdlib"),
        "_srcs": attr.label(default = "//v3/cmd/gazelle:gazelle_lib"),
    },
    executable = True,
